		...
		<h1>"*"표시는 필수 입력 항목입니다.</h1>
			<form method="post" action="UpdateMember.do" name="update">
				<table>	
					<tr>
						<td>이름 : </td>
						<td><input type="text" name="name" size="20" value="${login.name }">*</td>
					</tr>
					<tr>
						<td>아이디 : </td>
						<td>
					...
					<tr>
						<td>비밀번호 : </td>
						<td><input type="password" name="pwd" value = "${login.pwd }"></td>
					</tr>
					<tr>
						<td>비밀번호 확인 : </td>
					...
					<tr>
						<td>주소</td>
						<td><input type="text" name="place" value = "${login.place }">*</td>
					</tr>
					<tr>
						<td>종족</td>
						<td>
							<c:choose>
								...
								외계인<input type="radio" name="tribe" value="외계인" checked>
								마법사<input type="radio" name="tribe" value="마법사">
								</c:when>
								<c:otherwise>
								외계인<input type="radio" name="tribe" value="외계인">
								마법사<input type="radio" name="tribe" value="마법사" checked>
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
					<input type = "hidden" name="num" value = "${login.num}">
					<tr><td><input type="submit" value="회원수정 하기" onclick="return joinCheck()"><input type="reset" value="초기화"></td></tr>
				</table>
			</form>
		</div>
